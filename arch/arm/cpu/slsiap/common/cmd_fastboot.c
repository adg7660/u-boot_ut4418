...
static int fboot_cmd_flash_mmc(const char *cmd, char * buf)
{
	struct fastboot_device *fd = f_devices;
	struct fastboot_part *fp;
	struct list_head *entry, *n;
		...
		list_for_each_safe(entry, n, head) {
			...
			/*	if ((fst->down_bytes > fp->length) && (fp->length != 0)) {
				...
				//	if (0 > fd->write_part(fp, p, fst->down_bytes))
					if (0 > fd->write_part(fp, p, fp->length))
...
static int do_fastboot(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	if((argc==4)&&(!strcmp(argv[1],"flash"))){
		printf("enter fastboot test: %s %s %s %s\n",argv[0],argv[1],argv[2],argv[3]);
		mdelay(500);
		buf =  simple_strtoul(argv[3], NULL, 16);
		fboot_cmd_flash_mmc(argv[2],(char *)buf);
	//	fboot_lcd_stop();
		printf("fastboot test end\n");
		return 0;
	}
