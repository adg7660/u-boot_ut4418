...
/****************************************
**uboot\include\power\axp228.h 中修改充电电流
**  设置充电电流函数
**
**
** CHARGE_CURRENT				1050*1000
** POWEROFF_CHARGE_CURRENT		1500*1000=1500000
*****************************************/
...
static int axp228_param_setup(struct axp228_power *power)
{
	...
	val &= 0xE0;
	val |= 	( AXP_DCDC1_MODE_ENABLE << AXP_DCDC1_MODE_BIT)    //  0   0
			|(AXP_DCDC2_MODE_ENABLE << AXP_DCDC2_MODE_BIT)    //  1   1
			|(AXP_DCDC3_MODE_ENABLE << AXP_DCDC3_MODE_BIT)    //  1   2
			|(AXP_DCDC4_MODE_ENABLE << AXP_DCDC4_MODE_BIT)    //  1   3 
			|(AXP_DCDC5_MODE_ENABLE << AXP_DCDC5_MODE_BIT);   //  0   4
	axp228_i2c_write(power, AXP22_DCDC_MODESET, val);

	/* REG 10H: DCDC1/2/3/4/5&ALDO1/2&DC5LDO Enable Set */
	val = 	( AXP_ALDO2_ENABLE << AXP_ALDO2_EN_BIT)     //          7
			|(AXP_ALDO1_ENABLE << AXP_ALDO1_EN_BIT)     //          6
			|(AXP_DCDC5_ENABLE << AXP_DCDC5_EN_BIT)     //          5
			|(AXP_DCDC4_ENABLE << AXP_DCDC4_EN_BIT)     //          4
			|(AXP_DCDC3_ENABLE << AXP_DCDC3_EN_BIT)     //          3
			|(AXP_DCDC2_ENABLE << AXP_DCDC2_EN_BIT)     //          2
			|(AXP_DCDC1_ENABLE << AXP_DCDC1_EN_BIT)     //          1
			|(AXP_DC5LDO_ENABLE << AXP_DC5LDO_EN_BIT);  //          0
	axp228_i2c_write(power, AXP22_LDO_DC_EN1, val);

	/* REG 12H: Power Output Control */
	val = 	( AXP_DC1SW_ENABLE << AXP_DC1SW_EN_BIT)      //         7
			|(AXP_DLDO4_ENABLE << AXP_DLDO4_EN_BIT)      //         6
			|(AXP_DLDO3_ENABLE << AXP_DLDO3_EN_BIT)      //         5
			|(AXP_DLDO2_ENABLE << AXP_DLDO2_EN_BIT)      //         4
			|(AXP_DLDO1_ENABLE << AXP_DLDO1_EN_BIT)      //         3
			|(AXP_ELDO3_ENABLE << AXP_ELDO3_EN_BIT)      //         2
			|(AXP_ELDO2_ENABLE << AXP_ELDO2_EN_BIT)      //         1
			|(AXP_ELDO1_ENABLE << AXP_ELDO1_EN_BIT);     //         0
